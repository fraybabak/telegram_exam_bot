...
class BinaryAnswerRepository(Repository):
    ...
    def create(self, answer, question_id, user_id, campaign_id) -> BinaryAnswer:
        ...
        if binaryAnswer_model:
            raise Exception("BinaryAnswer already exists")
        binaryAnswer_model = self.model(
            answer=answer, binary_question_id=question_id, user_id=user_id, binary_campaign_id=campaign_id)
        self.db.add(binaryAnswer_model)
        self.db.commit()
        self.db.refresh(binaryAnswer_model)
        return BinaryAnswer(id=binaryAnswer_model.id, answer=binaryAnswer_model.answer, question_id=binaryAnswer_model.binary_question_id, user_id=binaryAnswer_model.user_id, campaign_id=binaryAnswer_model.binary_campaign_id) 
    ...
    def find_by_user(self, user_id: int) -> List[BinaryAnswer]:
        ...
        if binaryAnswer_model is None:
            raise Exception("BinaryAnswer not found")
        return [BinaryAnswer(id=binaryAnswer_model.id, answer=binaryAnswer_model.answer, question_id=binaryAnswer_model.binary_question_id, user_id=binaryAnswer_model.user_id, campaign_id=binaryAnswer_model.binary_campaign_id) for binaryAnswer_model in binaryAnswer_model]
    ...
    def find_by_question_and_user(self, question_id: int, user_id: int) -> BinaryAnswer:
        binaryAnswer_model = self.db.query(self.model).filter(
            self.model.binary_question_id == question_id, self.model.user_id == user_id).first()
        if binaryAnswer_model is None:
            raise Exception("BinaryAnswer not found")
        return BinaryAnswer(id=binaryAnswer_model.id, answer=binaryAnswer_model.answer, question_id=binaryAnswer_model.binary_question_id, user_id=binaryAnswer_model.user_id, campaign_id=binaryAnswer_model.binary_campaign_id)
    ...
    def find_by_question_and_campaign_and_user(self, question_id: int, campaign_id: int, user_id:id) -> List[BinaryAnswer]:
        binaryAnswer_model = self.db.query(self.model).filter(
            self.model.binary_question_id == question_id, self.model.binary_question_id == campaign_id, self.model.user_id == user_id).all()
        if binaryAnswer_model is None:
            raise Exception("BinaryAnswer not found")
        return [BinaryAnswer(id=binaryAnswer_model.id, answer=binaryAnswer_model.answer, question_id=binaryAnswer_model.binary_question_id, user_id=binaryAnswer_model.user_id, campaign_id=binaryAnswer_model.binary_campaign_id) for binaryAnswer_model in binaryAnswer_model]
